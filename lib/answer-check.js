...
function classifyBoldRuns(text, bold, start, end) {
  ...
  for (const sp of spans) {
    ...
    else { cur = { isBold, words: [sp.word], start: sp.start, end: sp.end }; runs.push(cur); }
  }
  // A [BOLD run] [single-word non-bold run wrapped in "(...)"] [BOLD run]
  // triple (e.g. the bold spans either side of "(Walt)" in `I'm going to
  // (Walt) Disney World!`) is a structurally different signal from an
  // ordinary un-bolded gap: it's an inline optional word spliced mid-
  // phrase, not a boundary between two independently-sufficient answers
  // (contrast "Green Bay" + "Packers", an ordinary un-bolded whitespace
  // gap with no parens). Fold the triple into one continuous required
  // run, keeping the optional word separately so both "with" and
  // "without" variants get accepted, rather than treating the two bold
  // sides as independently-sufficient answers on their own.
  for (let i = 1; i < runs.length - 1; i++) {
    const prev = runs[i - 1], mid = runs[i], next = runs[i + 1];
    if (!prev.isBold || mid.isBold || !next.isBold || mid.words.length !== 1) continue;
    if (!/^\s*\([^()]*\)\s*$/.test(text.slice(prev.end, next.start))) continue;
    prev.words.push(...next.words);
    prev.end = next.end;
    (prev.optional = prev.optional || []).push(mid.words[0]);
    runs.splice(i, 2);
    i--;
  }
  ...
  const required = [];
  for (const r of runs.filter((r) => r.isBold)) {
    required.push(r.words.join(" "));
    if (r.optional) required.push([...r.words, ...r.optional].join(" "));
  }
  return {
    required,
...
export function parseAnswerLine(answerLine, answerDisplay) {
  ...
  // The main answer ends at the first *directive-shaped* bracket/paren
  // (do-not/reject/prompt-on/or/accept/also-accept), not just the first
  // paren of any kind — a paren embedded mid-phrase that isn't
  // directive-shaped (e.g. "I'm going to (Walt) Disney World!") is an
  // inline optional word, not a directive boundary. Stopping at ANY paren
  // used to silently truncate everything after the first embedded
  // optional word out of the parsed answer entirely.
  let mainEnd = line.length;
  {
    const blockRe = /\[([^\]]*)\]|\(([^)]*)\)/g;
    blockRe.lastIndex = mainStart;
    let bm;
    while ((bm = blockRe.exec(line))) {
      const inner = bm[1] ?? bm[2];
      if (BARE_DIRECTIVE_RE.test(inner)) { mainEnd = bm.index; break; }
    }
  }
  const main = line.slice(mainStart, mainEnd).trim();
  if (main) {
    // split top-level "X or Y" into separate full alternatives (e.g. two
    // complete alternate phrases, not an accept-clause "or"), and expand
    // each alternative's own inline "(word)" optionals into "with" and
    // "without" variants — same droppable-parenthetical convention as the
    // leading-optional case above, just not anchored to the very start.
    for (const alt of splitOr(main)) {
      const stripped = alt.replace(/\([^)]*\)/g, " ").replace(/\s+/g, " ").trim();
      const kept = alt.replace(/[()]/g, " ").replace(/\s+/g, " ").trim();
      add(stripped, explicitAccept);
      if (kept !== stripped) add(kept, explicitAccept);
      if (leadingOptText) add(leadingOptText.trim() + " " + stripped, explicitAccept);
    }
...
// The "main" (non-directive) answer phrase — skips a leading "(Optional
// word)" and stops at the first directive-shaped bracket/paren, exactly
// like parseAnswerLine's own main-phrase extraction above. Exported so
// callers that need this substring (e.g. validate-question.mjs's
// self-check, which judges this text against the full answer line) can't
// drift out of sync with parseAnswerLine's actual boundary logic — that
// happened once already when validate-question.mjs carried its own
// separate copy of the old (buggy) "stop at ANY paren" version.
export function mainAnswerText(answerLine) {
  ...
  return line.slice(mainStart, mainEnd).trim();
}
